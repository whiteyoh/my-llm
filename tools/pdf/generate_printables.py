...
ALIASES = {
    "teacher_guide": ["teacher_guide.pdf", "Kairo_Teacher_Guide.pdf"],
    "student_worksheet": ["student_worksheet.pdf", "Kairo_Student_Worksheet.pdf"],
...
def printable_lines(src: str) -> list[str]:
    """Remove repository navigation HTML that is useful online but noisy on paper."""
    lines: list[str] = []
...
def draw_page(canvas, doc) -> None:  # type: ignore[no-untyped-def]
    ...
    canvas.setTitle(f"Kairo {title}")
    canvas.setAuthor("Kairo")
    canvas.setSubject("Kairo classroom printable")
    canvas.setCreator("Kairo printable generator")
    ...
    canvas.drawString(doc.leftMargin, page_height - 11 * mm, f"Kairo | {title}")
